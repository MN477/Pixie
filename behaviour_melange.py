"""
=============================================================================
behaviour_classifier_visual.py  —  v6  (unified knee + desk pipeline)
=============================================================================
WHAT THIS SCRIPT DOES
---------------------
Reads raw_body_multi.csv, seeks the video to each CSV frame_id, runs the
full 5-behaviour pipeline, draws results on-screen, writes two CSVs.

POSTURE DETECTION STRATEGY (automatic per-person per-frame)
------------------------------------------------------------
The classifier uses the BEST available evidence, automatically:

  IF knees AND ankles are visible  (full-body view, standing classroom)
  → KneePostureClassifier fires:
      • hip-knee-ankle angle  → sitting / standing
      • hysteresis state machine prevents flicker
      • uses MIN(left, right) knee angle for sitting sensitivity

  ELSE  (knees hidden behind desk — typical classroom seated view)
  → DeskPostureStateMachine fires:
      • shoulder-to-desk gap   → standing if > 1.2 sw above desk
      • head-to-desk gap       → slouching if < SLOUCH_HEAD_FLOOR sw above desk
      • forward head shift     → slouching supplement
      • body elongation        → standing supplement
      • DeskEstimator: rolling median of wrist/elbow Y (desk surface)

Both paths write to the SAME output columns.  The active path is logged
in the "posture_method" column ("knee" / "desk" / "unknown").

BOUNCING vs BOUNDING
---------------------
  bouncing  : FFT + variance + ZCR on head-Y (seated rhythmic oscillation)
  bounding  : frame-to-frame hip displacement (jumping / leaping — full-body)
Both detectors run every frame; bounding takes priority in dominant_behaviour.

CONTROLS
--------
  SPACE  pause/resume   Q/ESC  quit   S  snapshot   +/-  speed
...
class CFG:
    ...
    # Distinct from bouncing: fires on large single-frame hip displacement.
    # Both detectors run; bounding takes priority in dominant_behaviour.
    BOUND_DISP_NORM:      float = 0.25   # displacement / sw per frame → bounding
    BOUND_CONFIRM_FRAMES: int   = 3      # frames of consecutive displacement
    BOUND_DECAY_FRAMES:   int   = 8      # frames label stays active after last hit
    BOUND_HISTORY_LEN:    int   = 5      # smoothing window (rolling max)

    # ── Knee-based posture (fires when knees/ankles visible) ─────────────
    # Uses hip-knee-ankle angle with hysteresis.
    KNEE_SITTING_MAX:    float = 140.0   # enter sitting if avg_knee < this
    KNEE_SITTING_EXIT:   float = 150.0   # leave sitting if avg_knee > this
    KNEE_STANDING_MIN:   float = 158.0   # enter standing if avg_knee > this
    KNEE_STANDING_EXIT:  float = 148.0   # leave standing if avg_knee < this
    KNEE_CONFIRM_FRAMES: int   = 4       # frames posture must hold before switching
    # Confidence in knee-based result over desk-based (0 = always prefer desk,
    # 1 = always prefer knee when knees visible)
    KNEE_PRIORITY:       float = 1.0     # knees always win when visible
    ...
    # ── Display ────────────────────────────────────────────────────────────
    WINDOW_NAME: str = "Behaviour Classifier v6  —  Unified Knee + Desk"
    ...
    BEHAVIOUR_COLORS: dict = {
        ...
        "slouching":   (0,    80, 255),
        "bouncing":    (255, 140,   0),
        "bounding":    (0,  230, 255),   # bright cyan — large displacement
...
def _body_ruler(kpb: KeypointBuffer) -> float:
    ...
    Falls back to torso height (shoulder-to-hip) if shoulders are not
    both visible — useful when one shoulder is occluded.
    """
    ...
    # Fallback: shoulder-to-hip distance on whichever side is visible.
    # Shoulder width ≈ 0.8 × torso height in typical human proportions.
...
def _hip_centroid(kpb: KeypointBuffer) -> Optional[tuple[float, float]]:
    """Returns the midpoint of both hips, or whichever hip is available."""
...
def _knees_visible(kpb: KeypointBuffer) -> bool:
    """
    Returns True if at least one complete leg chain (hip-knee-ankle) is
    visible with sufficient confidence to compute a knee angle.
    Used to decide which posture classifier to activate.
    """
    for h_idx, k_idx, a_idx in [
        (KP_LEFT_HIP,  KP_LEFT_KNEE,  KP_LEFT_ANKLE),
        (KP_RIGHT_HIP, KP_RIGHT_KNEE, KP_RIGHT_ANKLE),
    ]:
        if (kpb.get(h_idx) is not None and
                kpb.get(k_idx) is not None and
                kpb.get(a_idx) is not None):
            return True
    return False
...
# ==============================================================================
# KNEE-BASED POSTURE CLASSIFIER  (fires when knees/ankles are visible)
# ==============================================================================
...
class KneePostureClassifier:
    """
    Stateful posture classifier using hip-knee-ankle angle.
    Activates only when at least one full leg chain is visible.

    Uses the MINIMUM of left/right knee angle so that a clearly-bent
    leg on one side overrides a missing or occluded other side.

    Hysteresis prevents flicker at sitting/standing boundaries:
      • Enter sitting  when min_angle < KNEE_SITTING_MAX
      • Leave sitting  when min_angle > KNEE_SITTING_EXIT
      • Enter standing when min_angle > KNEE_STANDING_MIN
      • Leave standing when min_angle < KNEE_STANDING_EXIT
    """
    ...
    def classify(self, kpb: KeypointBuffer) -> KneePostureResult:
        ...
        # Compute both knee angles (hip → knee → ankle)
        angles: list[float] = []
        for h_idx, k_idx, a_idx in [
            (KP_LEFT_HIP,  KP_LEFT_KNEE,  KP_LEFT_ANKLE),
            (KP_RIGHT_HIP, KP_RIGHT_KNEE, KP_RIGHT_ANKLE),
        ...
        if not angles:
            return res   # no visible legs — caller will fall back to desk path

        res.knee_angle_left  = angles[0] if len(angles) >= 1 else float("nan")
        res.knee_angle_right = angles[1] if len(angles) >= 2 else float("nan")
        # Use MINIMUM: if one leg is clearly bent, the person is sitting/crouching
        min_angle = float(np.min(angles))
        res.avg_knee_angle = min_angle

        # ── Raw label from angle ─────────────────────────────────────────
        if min_angle < CFG.KNEE_SITTING_MAX:
            raw = "sitting"
        elif min_angle > CFG.KNEE_STANDING_MIN:
            raw = "standing"
        else:
            # Ambiguous zone — hold previous state
            raw = self._state if self._state != "unknown" else "sitting"

        # ── Hysteresis exit conditions ────────────────────────────────────
        # Already sitting: only leave if angle clearly rises above exit threshold
        ...
        # Already standing: only leave if angle clearly drops below exit threshold
...
class BoundingDetector:
    """
    Detects large single-frame body displacements (jumps, leaps, large steps).
    Complements the seated BouncingDetector — fires on full-body movement.

    Strategy:
      • Track hip centroid each frame, normalised by shoulder width.
      • Smooth over BOUND_HISTORY_LEN frames using rolling MAX (catches peaks).
      • Trigger when smoothed displacement ≥ BOUND_DISP_NORM.
      • Decay counter keeps label active for BOUND_DECAY_FRAMES after last hit.

    Unlike the FFT bouncing detector (needs a full window to fire), this
    responds within 1–2 frames of the jump — essential for fast movements.
    """
    ...
    def update(self, kpb: KeypointBuffer, sw: float) -> tuple[float, int]:
        ...
        # Rolling MAX over the window: captures the peak displacement
...
class TrackState:
    """
    Per-track state.  Holds all detectors; process() picks the right posture
    path automatically based on keypoint availability each frame.
    """
    track_id:    int
    ...
    def process(self, kp_array: np.ndarray) -> dict:
        """
        Unified 5-behaviour pipeline.
        Routing logic:
          • If knees + ankles are visible → KneePostureClassifier (most accurate)
          • Otherwise                    → DeskPostureStateMachine (classroom fallback)
        All other detectors (hand, bouncing, bounding) run unconditionally.
        """
        # 1. Kalman smooth + interpolate
        self.kpb.update(kp_array)
        sw = _body_ruler(self.kpb)

        # 2. Hand raise first (desk estimator needs to know)
        self.hand_sm.update(self.kpb, sw)

        # 3. Desk estimate (skips hand-raised frames)
        self.desk.update(self.kpb, sw=sw, hand_raised=bool(self.hand_sm.raised))

        # 4. Posture — choose path
        use_knees     = _knees_visible(self.kpb)
        posture_method = "unknown"

        if use_knees:
            # ── Knee path ────────────────────────────────────────────────
            kr = self.knee_cls.classify(self.kpb)
            # Also run desk path (updates its EMA, but label overridden below)
            dr = self.posture_sm.update(self.kpb, sw, self.desk.desk_y)

            # Knee result is authoritative; merge raw desk signals for CSV
            posture_label      = kr.label
            posture_confidence = kr.confidence
            posture_method     = "knee"

            # Slouching from desk path can override even when knees visible
            # (a person can be sitting with bent knees AND slouching forward)
            ...
            # Expose both sets of raw signals
        ...
        # 5. Bouncing (seated oscillation — FFT + variance + ZCR on head-Y)
        bounce_score, is_bouncing = self.bounce.update(self.kpb, sw)

        # 6. Bounding (full-body displacement — hip centroid jump)
...
def dominant_behaviour(feat: dict) -> tuple[str, float]:
    """
    Priority order: hand_raised > bounding > bouncing > posture
    Bounding (full-body jump) takes precedence over seated bouncing.
    """
    if feat["hand_raised"]:  return "hand_raised", 1.0
    if feat["bounding"]:     return "bounding",    float(feat.get("bound_score",  0.0))
...
class Renderer:
    ...
    # Skeleton links — upper body always drawn; leg links drawn when knees visible
    _LINKS_UPPER = [
    ...
    def draw_skeleton(
        ...
        # Leg links (drawn only when the joints are actually visible)
    ...
    def draw_diagnostics(
        ...
        """Panel showing active posture method + all signals."""
        ...
        lines = [
            f"mode : {method}",
            # Knee signals (only meaningful when method=knee)
            f"knee : {_s('avg_knee_angle')}°",
            # Desk signals
            f"desk_y: {_s('desk_y_px',1)}px",
            f"spine : {_s('spine_tilt_deg')}°",
            f"hd_dk : {_s('head_desk_norm')}",
            f"sh_dk : {_s('shoulder_desk_norm')}",
            # Scores (highlighted when active)
        ...
        lh = 13; pw = 155; ph = lh * len(lines) + 6
        ...
        # Colour coding: knee mode = green tint, desk mode = cyan tint
        mode_col = (80, 255, 80) if method == "knee" else (255, 220, 80)
        score_keys = {6: "slouch_score", 7: "stand_score",
                      8: "bounce_score", 9: "bound_score", 10: "hand_score"}
        ...
        for i, line in enumerate(lines):
            ...
            if i in score_keys:
                try:
                    if float(feat.get(score_keys[i], 0.0)) > 0.30:
                        col = (80, 220, 255)
                except (ValueError, TypeError):
                    pass
    ...
    def draw_bounding_banner(
        ...
        """Flashing BOUNDING banner — fires on large body displacement."""
        if (tick // 10) % 2 == 0:
    ...
    def render_all(
        ...
        for tid, (track, feat) in results.items():
            ...
            # Desk surface line (only when desk path active)
            if method == "desk" and track.desk.is_ready:
...
_RAW_COLS = [
    ...
    # Knee signals (filled when knees visible)
    ...
    # Bouncing (seated oscillation)
    "bounce_score", "bouncing",
    # Bounding (full-body displacement)
